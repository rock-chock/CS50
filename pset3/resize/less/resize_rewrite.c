...
int main(int argc, char *argv[])
{
    ...
    // get array for storing triples of outfile width
    RGBTRIPLE array [bi_out.biWidth];
    ...
    for (int i = 0, biHeight = abs(bi_in.biHeight); i < biHeight; i++)
    {
        ...
        for (int j = 0; j < bi_in.biWidth; j++)
        {
            ...
            // put triple to array n times
            for (int l = j * n, next_n = l + n; l < next_n; l++)
            {
                array[l] = triple;;
            }
        }
        ...
        // write array and padding n times
        for (int p = 0; p < n; p++)
        {

            // write each item of array to outptr
